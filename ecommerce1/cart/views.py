...
def orderform(request):
    if(request.method=="POST"):
        ...
        if(ac.amount>=total):
            ...
            ac.save()
            for i in cart: # create records in order table for each object in cart table for the current user
                ...
                i.product.save()
            cart.delete() # clears the cart items for thr current user
            msg="Order Placed Successfully"
            return render(request,'orderdetail.html',{'m':msg})
        else:
            msg="Insuffcient Amount in User Account. You Cannot Place Order"
            return render(request, 'orderdetail.html', {'m': msg})
